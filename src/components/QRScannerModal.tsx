...
export function QRScannerModal({ open, onClose, onScan }: { open: boolean; onClose: () => void; onScan?: (batchCode: string) => void }) {
  ...
  if (!open) return null;
  function submitManual(event: FormEvent<HTMLFormElement>) { event.preventDefault(); goToTrace(manualCode); }
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Scan food QR code</h2><button onClick={onClose} className="text-slate-500">Close</button></div><div id={scannerId} className="mt-4 overflow-hidden rounded-lg" />{error && <p className="mt-3 rounded bg-amber-50 p-3 text-sm text-amber-900">{error}</p>}<form onSubmit={submitManual} className="mt-4 space-y-2"><label className="font-semibold">Or enter Batch ID</label><input value={manualCode} onChange={(event) => setManualCode(event.target.value)} placeholder="SK-2026-X1Y2" className="w-full rounded-lg border border-slate-300 p-3" /><button type="submit" className="w-full rounded-lg bg-green-700 py-3 font-bold text-white">Trace batch</button></form></div></div>;
}
