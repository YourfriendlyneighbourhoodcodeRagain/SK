...
export default function AggregatorDashboard() {
  ...
  async function lookupBatch(code: string, target: 'receive' | 'lab' | 'sell') {
    ...
    if (target === 'sell') {
      ...
      const { data: previousSales } = await supabase.from('batch_handoffs').select('quantity_kg').eq('batch_id', data.id).eq('stage', 'distribution').not('quantity_kg', 'is', null);
...
function SalesHistory() {
  ...
  useEffect(() => {
    ...
    async function loadSales() {
      ...
      if (!user) return;
      const { data } = await supabase.from('batch_handoffs').select('id, batch_id, assigned_distributor_name, quantity_kg, notes, created_at, batches(batch_code, crop_name)').eq('handler_id', user.id).eq('stage', 'distribution').not('quantity_kg', 'is', null).order('created_at', { ascending: false });
