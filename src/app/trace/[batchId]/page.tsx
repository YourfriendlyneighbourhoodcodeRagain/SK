...
import { formatHash, type BatchVerificationSummary } from '@/lib/blockchain';

export default async function TracePage({ params }: { params: Promise<{ batchId: string }> }) {
  const { batchId } = await params;
  let batch = null;
  let handoffs: BatchHandoff[] = [];
  let verificationSummary: BatchVerificationSummary | null = null;

  try {
    const trace = await getBatchByCode(batchId);
    batch = trace?.batch ?? null;
    handoffs = trace?.handoffs ?? [];
    verificationSummary = trace?.verificationSummary ?? null;
  } catch (err) {
    console.error('Error fetching batch trace:', err);
  }
  ...
  if (!batch) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-slate-50 p-6 text-center">
        ...
        <p className="mt-2 text-slate-600">This trace record is unavailable or its Batch ID is incorrect.</p>
        <Link href="/" className="mt-6 font-semibold text-green-700 hover:underline">
          Return to tracking
        </Link>
      </main>
    );
  ...
  const labStatus =
    batch.lab_tests?.find((test) => test.test_status === 'FAIL')?.test_status ??
    batch.lab_tests?.[0]?.test_status ??
    batch.pesticide_status ??
    'PENDING';
