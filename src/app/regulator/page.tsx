...
import type { BatchIntegrity } from '@/lib/integrity';
...
type Handoff = { id: string; created_at: string; stage: string; assigned_distributor_name?: string | null; assigned_retailer_name: string | null; notes: string | null; handler_id: string; location: string | null; prev_hash: string | null; current_hash: string | null; ledger_version: number };
type Batch = {
...
export default function RegulatorDashboard() {
  ...
  const [integrity, setIntegrity] = useState<Record<string, BatchIntegrity>>({});
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState<'all' | 'recalls' | 'failed' | 'integrity'>('all');
  ...
  const loadBatches = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('batches')
      .select('id, batch_code, crop_name, farm_location, status, is_recalled, created_at, lab_tests(test_status, residue_ppm, max_limit_ppm, lab_name, certificate_url, created_at), batch_handoffs(id, created_at, stage, assigned_distributor_name, assigned_retailer_name, notes, handler_id, location, prev_hash, current_hash, ledger_version)')
      .order('created_at', { ascending: false });
    if (error) setMessage(error.message);
    else {
      const loaded = (data ?? []) as Batch[];
      setBatches(loaded);
      const results = await Promise.all(loaded.map(async (batch) => {
        const response = await fetch(`/api/integrity/${encodeURIComponent(batch.id)}`);
        return response.ok ? [batch.id, await response.json() as BatchIntegrity] as const : null;
      }));
      setIntegrity(Object.fromEntries(results.filter((result): result is readonly [string, BatchIntegrity] => result !== null)));
      setMessage('');
    }
    setLoading(false);
  }, []);
  ...
  const chainStatus = (batch: Batch) => integrity[batch.id]?.status ?? 'WARNING';
  ...
  const visibleBatches = batches.filter((batch) => {
    ...
    const matchesPrimaryFilter = filter === 'all' || (filter === 'recalls' && batch.is_recalled) || (filter === 'failed' && failedTest(batch)) || (filter === 'integrity' && chainIssue(batch));
    ...
    <PortalShell title="Regulator compliance console" description="Food safety oversight and traceability audit." icon={ShieldCheck}>
      <div className="space-y-6">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <button type="button" onClick={() => setFilter('integrity')} className="text-left"><StatCard label="Chain warnings" value={chainIssues} icon={<ShieldCheck className="text-red-700" />} tone={chainIssues ? 'red' : 'default'} /></button>
        </section>
