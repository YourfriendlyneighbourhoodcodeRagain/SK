...
export default function RegisterPage() {
  ...
  return <PublicShell><Card className="portal-surface w-full max-w-lg"><CardHeader className="text-center"><Link href="/" className="mx-auto flex items-center gap-2 text-green-700"><Leaf /><span className="text-xl font-bold">SurakshaKhadya</span></Link><CardTitle className="pt-4 text-2xl">Create your account</CardTitle><CardDescription>Join the food safety traceability network.</CardDescription></CardHeader><CardContent><form onSubmit={handleSubmit} className="space-y-4">{error && <p role="alert" className="portal-status-error p-3 text-sm">{error}</p>}{notice && <p role="status" className="portal-status-success p-3 text-sm">{notice}</p>}<div className="space-y-2"><Label htmlFor="fullName">Full name / organisation</Label><Input className="portal-field" id="fullName" value={fullName} onChange={(event) => setFullName(event.target.value)} required /></div><div className="space-y-2"><Label htmlFor="role">Role</Label><select id="role" value={role} onChange={(event) => setRole(event.target.value as Role)} className="portal-field flex h-10 w-full rounded-md px-3 text-sm"><option value="farmer">Farmer</option><option value="aggregator">Aggregator</option><option value="distributor">Distributor</option><option value="retailer">Retailer</option></select></div><div className="space-y-2"><Label htmlFor="location">Location / district</Label><Input className="portal-field" id="location" value={location} onChange={(event) => setLocation(event.target.value)} required /></div><div className="space-y-2"><Label htmlFor="email">Email</Label><Input className="portal-field" id="email" type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></div><div className="space-y-2"><Label htmlFor="password">Password</Label><Input className="portal-field" id="password" type="password" autoComplete="new-password" minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} required /><p className="text-xs text-slate-500">At least 6 characters.</p></div><Button type="submit" disabled={isSubmitting} className="portal-action w-full py-6 text-lg">{isSubmitting ? 'Creating account…' : <>Create account <ArrowRight className="ml-2" /></>}</Button></form><p className="mt-6 text-center text-sm text-slate-600">Already have an account? <Link href="/login" className="font-semibold text-green-700 hover:underline">Sign in</Link></p></CardContent></Card></PublicShell>;
}
