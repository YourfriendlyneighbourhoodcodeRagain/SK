...
function initialState(): RetailerState {
  if (typeof window === 'undefined') return { deliveries: [], stock: [], orders: [] };
  try { const saved = window.localStorage.getItem(storageKey); return saved ? JSON.parse(saved) as RetailerState : { deliveries: [], stock: [], orders: [] }; }
  catch { return { deliveries: [], stock: [], orders: [] }; }
}
function displayTime(date: Date) { return date.toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }); }
function purchaseId(delivery: RetailerDelivery) { return `PUR-${delivery.id.replace('delivery-', '')}`; }
function orderId(sale: Sale) { return `ORD-${sale.id.replace('sale-', '')}`; }
...
export default function RetailerDashboard() {
  ...
  const [view, setView] = useState<'purchases' | 'orders' | 'stock' | null>(null);
  ...
  return <PortalShell title="Retailer Dashboard" description="Manage your deliveries and orders." icon={Store} showWorkspaceLink={false} contentClassName="bg-[radial-gradient(circle_at_top_right,_rgba(220,252,231,0.7),_transparent_34%)]">
    ...
    <AddOrderDialog open={addOrderOpen} stock={data.stock} onClose={() => setAddOrderOpen(false)} onSave={(newOrder: Sale) => { setData((current) => { const orders = current.orders ?? []; return { ...current, orders: [...orders, { ...newOrder, id: `sale-${String(orders.length + 1).padStart(3, '0')}` }] }; }); setAddOrderOpen(false); }} />
    <PurchaseDialog purchase={purchase} onShowQr={(batchId) => setQrBatchId(batchId)} onClose={() => setPurchase(null)} />
    <OrderDialog order={order} onClose={() => setOrder(null)} />
    <QRCodeModal batchId={qrBatchId} open={Boolean(qrBatchId)} onClose={() => setQrBatchId(null)} />
  </PortalShell>;
}
...
function StockDialog({ open, stock, onClose }: { open: boolean; stock: StockItem[]; onClose: () => void }) { return <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}><DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto p-6 sm:p-7"><DialogHeader><DialogTitle className="text-2xl">My Stock</DialogTitle><DialogDescription className="text-base">Products currently available in your shop.</DialogDescription></DialogHeader><div className="mt-2 space-y-3">{stock.map((item) => <StockRecord key={item.batchId} item={item} />)}</div>{!stock.length && <p className="py-8 text-center text-slate-500">No stock records yet.</p>}</DialogContent></Dialog>; }
function StockRecord({ item }: { item: StockItem }) { return <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-lg font-bold text-slate-900">{item.emoji} {item.product}</p><p className="mt-1 text-sm text-slate-500">Batch tracked product</p></div><StockStatusBadge status={item.status} /></div><div className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-2"><RecordDetail label="Quantity" value={`${item.quantity} kg`} /><RecordDetail label="Batch ID" value={item.batchId} /></div></div>; }
function StockStatusBadge({ status }: { status: StockStatus }) { const safe = status === 'AVAILABLE'; const low = status === 'LOW STOCK'; return <span className={`shrink-0 rounded-full px-3 py-1 text-sm font-bold ${safe ? 'bg-green-100 text-green-800' : low ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'}`}>{safe ? '✓ Available' : low ? '⚠ Low stock' : status === 'BLOCKED' ? '✕ Blocked' : 'Out of stock'}</span>; }
function PurchaseRecord({ purchase, onSelect }: { purchase: RetailerDelivery; onSelect: () => void }) { return <button onClick={onSelect} className="group block w-full rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-300 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-700 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-lg font-bold text-slate-900">{purchase.emoji} {purchase.product}</p><p className="mt-1 text-sm text-slate-500">Received {purchase.receivedAt ?? purchase.date}</p></div><StatusBadge status={purchase.status} /></div><div className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-3"><RecordDetail label="Distributor" value={purchase.distributor} /><RecordDetail label="Quantity" value={`${purchase.quantity} kg`} /><RecordDetail label="Batch ID" value={purchase.batchId} /></div></button>; }
function OrderRecord({ order, onSelect }: { order: Sale; onSelect: () => void }) { return <button onClick={onSelect} className="group block w-full rounded-xl border border-slate-200 bg-white p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-green-300 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-700 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-lg font-bold text-slate-900">{order.emoji} {order.product}</p><p className="mt-1 text-sm text-slate-500">{order.date}</p></div><StatusBadge status="SOLD" /></div><div className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-3"><RecordDetail label="Order ID" value={orderId(order)} /><RecordDetail label="Quantity" value={`${order.quantity} kg`} /><RecordDetail label="Batch ID" value={order.batchId} /></div></button>; }
function RecordDetail({ label, value }: { label: string; value: string }) { return <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-1 break-words text-base font-semibold text-slate-800">{value}</p></div>; }
function StatusBadge({ status }: { status: RetailerDelivery['status'] | 'SOLD' }) { const sold = status === 'SOLD'; return <span className={`shrink-0 rounded-full px-3 py-1 text-sm font-bold ${sold || status === 'RECEIVED' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{sold ? '✓ Sold' : status === 'RECEIVED' ? '✓ Received' : '⚠ Reported'}</span>; }
function AddOrderDialog({ open, stock, onSave, onClose }: { open: boolean; stock: StockItem[]; onSave: (order: Sale) => void; onClose: () => void }) {
  const availableStock = stock.filter((item) => item.status !== 'BLOCKED' && item.status !== 'OUT OF STOCK');
  const firstStock = availableStock[0];
  const [product, setProduct] = useState(firstStock?.product ?? '');
  const [customProduct, setCustomProduct] = useState('');
  const [quantity, setQuantity] = useState('');
  const [amount, setAmount] = useState('');
  const [batchId, setBatchId] = useState(firstStock?.batchId ?? '');
  const selectedStock = availableStock.find((item) => item.product === product);
  const saleProduct = product === '__other__' ? customProduct.trim() : product;

  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!saleProduct || !quantity || !amount || !batchId) return;
    onSave({ id: 'sale-new', product: saleProduct, emoji: selectedStock?.emoji ?? '📦', quantity: Number(quantity), amount: Number(amount), date: 'Today', batchId });
    setQuantity(''); setAmount(''); setCustomProduct(''); setProduct(firstStock?.product ?? ''); setBatchId(firstStock?.batchId ?? '');
  }
