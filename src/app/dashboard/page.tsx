...
export default function DashboardPage() {
  ...
  if (error) return <main className="flex min-h-screen items-center justify-center bg-slate-50 p-4"><Card className="max-w-lg"><CardContent className="p-6"><p className="text-red-700">{error}</p><Button className="mt-4" onClick={signOut}>Sign out</Button></CardContent></Card></main>;
...
function RoleActions({ role }: { role: Role }) {
  ...
  if (role === 'aggregator') return <div className="grid gap-5 md:grid-cols-2"><ActionCard title="Receive farmer batch" description="Scan or enter a farmer batch and record quality notes." href="/aggregator" icon={<QrCode className="h-8 w-8" />} color="bg-blue-700 hover:bg-blue-800" button="Scan Farmer Batch" /><ActionCard title="Lab safety records" description="Attach pesticide residue and lab certificate results." href="/aggregator" icon={<FlaskConical className="h-8 w-8" />} color="bg-blue-700 hover:bg-blue-800" button="Attach Lab Residue Test" /></div>;
  if (role === 'distributor') return <ActionCard title="Log transport handoff" description="Record origin, destination, storage temperature, and transport notes." href="/distributor" icon={<Truck className="h-8 w-8" />} color="bg-amber-600 hover:bg-amber-700" button="Log Transport Handoff" />;
  if (role === 'retailer') return <div className="grid gap-5 md:grid-cols-2"><ActionCard title="Store shelf acceptance" description="Receive a batch at your store and generate a consumer QR code." href="/retailer" icon={<Store className="h-8 w-8" />} color="bg-purple-700 hover:bg-purple-800" button="Store Shelf Acceptance" /><ActionCard title="Safety recall" description="Immediately flag contaminated batches and begin a recall." href="/retailer" icon={<ShieldAlert className="h-8 w-8" />} color="bg-red-600 hover:bg-red-700" button="Trigger Contamination Recall" /></div>;
  return <ActionCard title="Compliance console" description="Review the complete batch register, lab safety outcomes, and recall alerts." href="/regulator" icon={<ClipboardCheck className="h-8 w-8" />} color="bg-slate-800 hover:bg-slate-900" button="Open Regulator Console" />;
}
...
function ActionCard({ title, description, href, icon, color, button }: { title: string; description: string; href: string; icon: React.ReactNode; color: string; button: string }) {
  return <Card className="border-slate-200"><CardHeader><div className="mb-2 text-slate-700">{icon}</div><CardTitle>{title}</CardTitle><CardDescription>{description}</CardDescription></CardHeader><CardContent><Button render={<Link href={href} />} className={`w-full py-4 text-lg font-bold ${color}`}>{button}</Button></CardContent></Card>;
}
