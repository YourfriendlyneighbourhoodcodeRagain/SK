...
export async function createBatch(formData: BatchFormData): Promise<BatchRecord> {
  ...
  const { data, error } = await supabase.from('batches').insert({
    ...
    total_weight_kg: formData.totalWeightKg,
...
export async function getBatchByCode(batchCode: string): Promise<BatchTrace | null> {
  ...
  const { data: batch, error: batchError } = await supabase
    .from('batches')
    .select('*, lab_tests(test_status, residue_ppm, max_limit_ppm, certificate_url, created_at)')
  ...
  const handoffs = (rawHandoffs ?? []) as HandoffRecord[];
